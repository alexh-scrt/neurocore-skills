import asyncio
import logging
import yaml
from flowengine import FlowContext
from neurocore import AsyncSkill, SkillMeta

logger = logging.getLogger(__name__)

class WorkerSkill(AsyncSkill):
    skill_meta = SkillMeta(
        name="worker",
        version="0.1.0",
        description="Use neurogossip-agent-v3 to coordinate with other tasks via yaml passed to it",
        provides=["worker_output"],
        consumes=["task_yaml"],
        config_schema={
            "properties": {
                "worker_model": {
                    "type": "string",
                    "description": "Underlying model worker engine to run executions"
                },
                "persistence_root": {
                    "type": "string",
                    "description": "Target folder where execution artifacts are written"
                },
                "redis_url": {"type": "string"}
            }
        }
    )

    async def setup_gossip(self) -> None:
        import os as _os
        import redis.asyncio as aioredis
        from neurogossip_agent.session_manager import AgentConversationManager
        from neurogossip_agent.transport import RedisAgentTransport, MockAgentTransport
        
        redis_url = self.config.get("redis_url") or _os.environ.get("NEUROGOSSIP_V3_REDIS_URL") or "redis://localhost:6379/0"
        try:
            self.redis_client = aioredis.from_url(redis_url, decode_responses=True)
            await self.redis_client.ping()
            self.transport = RedisAgentTransport(agent_id=self.name, redis_url=redis_url)
            self.manager = AgentConversationManager(self.redis_client, self.transport)
        except Exception:
            self.redis_client = None
            self.transport = MockAgentTransport()
            self.manager = AgentConversationManager(None, self.transport)
        
        self.manager.register_handler(self._handle_gossip_message)
        await self.transport.connect()
        await self.manager.start_listening()

    async def teardown_gossip(self) -> None:
        if hasattr(self, "manager") and self.manager:
            await self.manager.stop_listening()
        if hasattr(self, "transport") and self.transport:
            await self.transport.disconnect()
        if hasattr(self, "redis_client") and self.redis_client:
            await self.redis_client.aclose()

    async def _handle_gossip_message(self, context, message_id, reply_to, tags, sender_id, payload) -> None:
        if "request" in tags:
            context_flow = FlowContext()
            context_flow.set("task_yaml", str(payload))
            res_ctx = await self.process(context_flow)
            await self.manager.send_response(request_id=message_id, sender_id=self.name, payload=res_ctx.get("worker_output"))

    async def process(self, context: FlowContext) -> FlowContext:
        await self.setup_gossip()
        
        task_yaml = context.get("task_yaml", "")
        model = self.config.get("worker_model", "default")
        persist_dir = self.config.get("persistence_root", "")
        
        if not task_yaml:
            context.set("worker_output", "Failed: missing task details.")
            await asyncio.sleep(0.05)
            await self.teardown_gossip()
            return context
        
        try:
            task_spec = yaml.safe_load(task_yaml)
            logger.info("Worker parsed task specification. Using model '%s', saving under '%s'", model, persist_dir)
            context.set("worker_output", f"Task step {task_spec.get('step_id', 'unknown')} executed successfully.")
        except Exception as e:
            logger.error("Worker task execution failed: %s", e)
            context.set("worker_output", f"Failed: {str(e)}")
        
        await asyncio.sleep(0.05)
        await self.teardown_gossip()
        return context
