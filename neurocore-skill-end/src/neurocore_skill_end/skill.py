...
class EndSkill(AsyncSkill):
    skill_meta = SkillMeta(
        name="end",
        version="0.1.0",
        description="Stop execution and pass control back to the master agent",
        provides=["flow_completed"],
        consumes=["flow_context"],
        config_schema={
            "properties": {
                "format_output": {
                    "type": "string",
                    "description": "Style format for output returned (raw | json | markdown_report)",
                    "default": "raw"
                },
    ...
    async def _handle_gossip_message(self, context, message_id, reply_to, tags, sender_id, payload) -> None:
        if "request" in tags:
            ...
            context_flow.set("flow_context", payload)
            res_ctx = await self.process(context_flow)
            await self.manager.send_response(request_id=message_id, sender_id=self.name, payload=res_ctx.get("flow_completed"))

    async def process(self, context: FlowContext) -> FlowContext:
        await self.setup_gossip()
        fmt = self.config.get("format_output", "raw")
        context.set("flow_completed", True)
        logger.info("NeuroCore execution flow completed. Format style: %s", fmt)
    ...
    def teardown(self, context: FlowContext) -> None:
        gc.collect()
        logger.info("NeuroCore components garbage collected and process memory reclaimed.")
